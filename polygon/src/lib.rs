...
pub fn find_lg_endp(polygon: &[f32], angle: f32) -> ((f32, f32), (f32, f32)) {
    ...
    let start_point = (polygon[min_index * 2], polygon[min_index * 2 + 1]);
    let end_point   = (start_point.0 + total_dot * direct_vec2[0], start_point.1 + total_dot * direct_vec2[1]);
...
pub fn split_by_lg_0(mut points: Vec<f32>, polygon_indices: &Vec<u16>, lg_pos: &[f32], start: (f32, f32), end: (f32, f32)) -> PolygonCfg {
    ...
    new_dot_list.push(get_dot(direct_vec2[0], direct_vec2[1], point0.0 - _start.0, point0.1 - _start.1));
    ...
    while index < src_polygon_point_count {
        ...
        if index < src_polygon_point_count - 1 {
            ...
            new_dot_list.push(get_dot(direct_vec2[0], direct_vec2[1], point1.0 - _start.0, point1.1 - _start.1));
        }
    ...
    while index < point_count {
        ...
        while attr_index < attr_count {
            ...
            while a_index < attr_size {
                ...
                attrs[attr_index][point_attr_index as usize + a_index] = pre_attr + (nxt_attr - pre_attr) * percent;
